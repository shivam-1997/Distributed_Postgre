...
/**
 * Make multiple query from single queries separated by semi-colon
 */
...
/**
 * Create Tables Function Added by Rahul Sharma
 */
...
char* query_rewrite(char *query){
	...
	else if(!strcmp("insert",token) || !strcmp("INSERT",token)){
...
InteractiveBackend(StringInfo inBuf)
{
	...
	//Lines added by Debayan
	/*FILE *fp = NULL;
	fp=fopen("Desktop/CS631/Project/temp/abc.txt","w");
	fprintf(fp,"%s\n",inBuf->data);
	fclose(fp);*/
	//char create[]="create";
	//char querynew[2000];
	/*if(!strncmp(create,inBuf->data,6))
	{
		char *table_name; // get table name
		int i;
		for(i=0;i<3;i++)
		{
	...
	printf("%c\n",inBuf->data[2]);
	*/
	//Lines added by Debayan end

	//Added by Rahul Sharma
	/*
	printf("inBuf Data kami : %s\n",(inBuf->data));
	char *original_query=(char*)malloc(sizeof(inBuf->data));
	strcpy(original_query,(inBuf->data));
	printf("Original Query : %s\n",original_query);
	char *modified_query = query_rewrite(original_query);
	printf("Modified Query : %s\n",modified_query);
	strcpy(inBuf->data,modified_query);
	*/
	//printf("inBuf Data : %s\n",inBuf->data);
	//End
